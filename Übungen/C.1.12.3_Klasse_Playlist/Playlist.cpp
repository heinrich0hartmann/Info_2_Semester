...
/*=====================================================================================*/
bool playlist::Abspielen(const string& titel)
/*=====================================================================================*/

{
	...
	if (titel.size() == false)
	{
		...
		struct titel* aktuellerTitel = start_pointer;
		if (aktuellerTitel == NULL)
		{
			//Playlist ist leer
			return false;
		}
		while (aktuellerTitel != NULL)
		{
			cout << aktuellerTitel->name << "," << aktuellerTitel->interpret << ",";
			cout << enumkat_in_string(aktuellerTitel->kategorie) << '\a' << endl;
			aktuellerTitel = aktuellerTitel->next;
			Sleep(2000);
		}
	}
	else
	{
		// übergebenen Titel abspielen
		if (TitelSuchenundAnzeigen(titel, interpret, kategorie) == false)
		{
			return false;
		}
		else
		{
			...
			cout << enumkat_in_string(kategorie) << '\a' << endl;
			// Hier müsste laut Stock clock() verwendet weden, aber diese Methode gibt lediglich einen Zeitpunkt zurück
			// und verzögert nicht wie gemeint die Programmausführung?!
			// clock();
			// Wir nutzen hier stattdessen lieber sleep();
			Sleep(2000);
		}
	}
	return true;
