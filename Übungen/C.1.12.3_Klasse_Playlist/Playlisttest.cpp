...
int main(void)
{
	...
	do {
		...
		case 4:
			...
			break;
		case 5:
			my_playlist.ZeileEingeben("M\224chten Sie die gesamte Playlist abspielen? (j/n): ", temp);

			if (temp == "n")
			{
				my_playlist.ZeileEingeben("Bitte den abzuspielenden Titel der Playlist eingeben:",
					Titel);
			}
			// Playlist abspielen
			if (my_playlist.Abspielen(Titel) == false)
			{
				cout << "\nDer angegebene Titel \"" << Titel << "\" wurde nicht gefunden "
					"oder die Playlist ist leer.\n";
				system("pause");
			}
			break;
		case 6:
